...
import {mergeToml, parseToml} from './toml.js'
...
function mergeCustomToml(generated: string, custom: string): string {
	if (!custom.trim()) return generated.endsWith('\n') ? generated : `${generated}\n`
	return mergeToml(generated, custom, '# Generated by Zcash Node (custom overrides merged).')
}
...
async function writeGeneratedFiles(settings: SettingsSchema): Promise<void> {
	...
	const merged = mergeCustomToml(generateManagedToml(settings), custom)
...
export async function updateCustomOptions(rawText: string): Promise<string> {
	...
	// Reject broken TOML with a 400 before anything is written or restarted.
	parseToml(userLines)
